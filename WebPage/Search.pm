package mobedac::WebPage::Search;
...
~ . $self->application->component('sAjax')->output;
  ...
  $html .= "<input id='tSimpleAll' type='text' placeholder='by metadata / mobedac id (name, biome, project name, 4441137.3), function or organism...' value='".$stext."' style='width:580px;'>";
  ...
  $html .= "&nbsp;&nbsp;<input type='checkbox' name='resType' id='metaSimple' checked/>&nbsp;<span style='font-weight: bold; color: ".$colors[0]."'>metadata / mobedac id</span>";
  ...
  $html .= "<label for='tSimpleMeta' style='font-weight:bold;'>Find by metadata / mobedac id<br></label>";
  $html .= "<input id='tSimpleMeta' type='text' placeholder='mobedac id, name, biome, project name...' value='' style='width:260px;'><br><button id='bSimpleMeta'>Search</button>";
  $html .= "</div>";
...
sub get_simple_table {
  ...
  my $cols = [ { name => 'Job Number', visible => 0, sortable => 1 },
	       ...
	       { name => 'mobedac ID', filter => 1, operator => 'like', visible  => 1 },
...
sub get_user_jobs {
  ...
  my $dbh = $mgrast->db_handle();
  my $statement = "select Job.metagenome_id from Job left outer join JobgroupJob on Job._id=JobgroupJob.job where JobgroupJob.jobgroup=".$sloan_group->_id;
  $mgs = [];
