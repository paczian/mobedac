package mobedac::WebPage::MetaDataMG;
...
sub init {
  ...
  my $coll   = undef;#$mddb->get_collection_for_job($job);
  my $prefix = "MetaData_";
...
~; 
  ...
  my $table;
  if ( $self->data('meta_data') && @{$self->data('meta_data')} ) {
    $table = $self->get_report_table( $self->data('meta_data'), {} );
  }
  elsif ( $prefill && ref($prefill) && $struct->{data} && @{$struct->{data}} ) {
    $table = $self->get_report_table( $struct->{data}, $prefill );
  }
  $table = "<b>no data available</b>" unless ($table);
...
sub get_report_table {
  ...
  my @table_data;
  foreach my $row (@$data) {
    my ($tag, $step, $title, $name, $val) = @$row;
    my $category = ($step eq $title) ? $step : "$step : $title";

    if ( exists $vals->{$tag} ) { $val = $vals->{$tag}; }
    if ( $val && (! ref $val) ) { $val = [ $val ]; }
    if ( ! $val ) { next; }
    foreach ( @$val ) {
      unless ( $_ && ($_ =~ /\S/) ) { next; }
      push @table_data, [ $tag, $category, $name, $_ ];
    }
  }
  unless (@table_data) { return 0; }
...
sub upload_meta_data {
  ...
  if ($fh) {
    while (my $line = <$fh>) {
      chomp $line;
      my ($key, $cat, $name, @val) = split(/\t/, $line);
      $prefill->{ $self->data('prefix').$cat."_".$key } = [ @val ];
    }
  }
  
  while ( my ($param, $value) = each %{$cgi->Vars} ) {
...
sub check_data {
  ...
  unless ( defined($value) && (ref($value) eq "ARRAY") && (@$value > 0) ) {
    ...
    return $checked, [], ([ $name, $display_cat, $display_text, [] ]);
  }
