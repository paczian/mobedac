package mobedac::WebPage::Download;
...
Download - displays download information about a metagenome job
...
sub public_project_download_table {
  ...
  my $sloan_jobgroupjobs = $jobdbm->JobgroupJob->get_objects( { jobgroup => $sloan_group } );
  my $sloan_jobs = {};
  %$sloan_jobs = map { $_->{job} => 1 } @$sloan_jobgroupjobs;

  my $public_jobs = {};
  my $list = $jobdbm->Job->get_objects( { public => 1 } );
  @$list = map { if ($sloan_jobs->{$_->{_id}}) {$sloan_jobs->{$_->{metagenome_id}} = 1 }; $sloan_jobs->{$_->{_id}} ? $_ : () } @$list;
  ...
  if ( scalar(@$projects) || scalar(@$public_projects) ) {
    ...
    foreach my $project (sort {$a->id <=> $b->id } @$projects) {
      ...
      @$jobs = map { $sloan_jobs->{$_->{job}} ? $_ : () } @$jobs;
