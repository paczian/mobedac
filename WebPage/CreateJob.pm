package mobedac::WebPage::CreateJob;
...
sub output {
    ...
    unless ($user) {
	return "<p>You must be logged in to upload metagenome files and create jobs.</p><p>Please use the login box in the top right corner or return to the <a href='mobedac.cgi'>start page</a>.</p>";
    }
...
<param name="uc_uploadUrl" value="$FIG_Config::cgi_url/mobedac.cgi">
...
# 		  'processing_error'       => [ '',                         ''],
	...
	if ( $processing ) {
	    my $rand_str = $self->random_string();
	    my $msg = qq(<a href="?page=Upload&step=$rand_str">Please reload this page periodically to view the progress of your uploaded data set);
	    if ( $processing > 1 ) {
		$msg .= 's';
	    }
	    $msg .= "</a>\n";
...
sub processing_completed_tables {
	...
	$html .= qq(<form id="$form_name" method="post" action="mobedac.cgi">\n);
	...
	$html .= qq(<input type='hidden' name='page' id='page' value='Upload'>\n);
...
# create job directory
	...
	$jobmaster->JobgroupJob->create( { jobgroup => $sloan, job => $job_object } );
	#####
...
sub new_upload {
  ...
  if (ref($job)) {
    $content = "<img src='./Html/clear.gif' onload='window.open(\"mobedac.cgi?page=MetaDataMG&from_upload=1&metagenome=".$job->genome_id."\");document.getElementById(\"metagenome_id\").value=\"".$job->genome_id."\";document.forms.upload_form.submit();'>";
  } else {
