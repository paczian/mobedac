package mobedac::WebPage::MetagenomeProject;
...
sub project_list {
  ...
  my $sloan_group = $jobdbm->Jobgroup->get_objects( { name => 'Sloan' } )->[0];
  my $sloan_jgjobs = $jobdbm->JobgroupJob->get_objects({ jobgroup => $sloan_group });
  my $sloan_jobs = {};
  %$sloan_jobs = map { $_->job->{_id} => 1 } @$sloan_jgjobs;
  ...
  if ( scalar(@$private_projects) || scalar(@$public_projects) ) {
	...
	if ($sloan_jobs->{$j->{job}}) {
      ...
      next unless scalar(@$sloan_pjs);
      my $pid   = $project->id;
      my $pdata = $project->data;
      my $name  = $pdata->{PI_lastname} ? $pdata->{PI_lastname}.($pdata->{PI_firstname} ? ', '.$pdata->{PI_firstname} : '') : '';
      push(@$data, [ $pid, "<a href='metagenomics.cgi?page=MetagenomeProject&project=$pid'>".$project->name."</a>", $name, scalar(@$sloan_pjs) ]);
    }
    $table->data($data);
...
sub job_list {
  ...
  my @pjobs    = map { $_->[0] } @$pdata;
  my $metadata = $self->data('mddb')->get_metadata_for_tables(\@pjobs, 1, 1);
...
sub export_metadata {
  ...
  my $html = "<p><b>click to download: </b><a href='metagenomics.cgi?page=MetagenomeProject&action=download_md&filetype=xlsx&filename=$base.xlsx'>MG-RAST metadata file</a></p>";
  unless ($is_valid) {
