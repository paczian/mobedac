package mobedac::WebPage::DownloadMetagenome;
...
~;
  ...
  unless ($job) {
    #$content .= "Global download in development.\n";
    ...
    #The metadata is available in GSC (link) compliant MiXS format.</p>";
    
    # $content .= "<table><tr><td rowspan=2>" . $self->public_project_download_table . "</td><td>Legend</td></tr>";
    # $content .= "<tr><td align='top'>Cool tools</td></td></table>";
...
<a href='ftp://ftp.metagenomics.anl.gov/metagenomes/$pid/$mid.processed.tar.gz'><img src='$FIG_Config::cgi_url/Html/mg-download.png' alt='Download all derived data for this metagenome' height='15'/><small>analysis</small></a>" ;
  ...
  $self->title("");
  $content .="<h1 style='display:inline'>Metagenome Downloads</h1>" . $download ;
...
sub read_stats {
  my ($self, $source_file, $type) = @_;

  my $stats      = [];
  my $stats_file = $source_file . ".stats";
  ...
  if ((! -s $stats_file) && ($type =~ /^(protein|dna)$/i)) {
    system("$FIG_Config::seq_length_stats --fasta_file $source_file --stat_file $stats_file $option");
  }
...
sub stage_download_info {
  ...
  foreach my $f (@fs) {
    ...
    elsif ($self->data('get_sims') && ($desc eq 'expand') && ($suffix ne '.ontology')) {
      $type = "Sims";
      $desc = "annotated";
    }
    else {
    ...
    $file_table .= join "\n", ("<tr><form name='stage$sid' id='stage$sid'>",
			       ...
			       "<input type='hidden' name='file' value='$fname$suffix'>",
			       "</td>",
...
sub public_project_download_table {
  ...
  my $jobGroup = $jobdbm->Jobgroup->init( { name => 'Sloan' });
  my $jgs      = $jobdbm->JobgroupJob->get_objects( { jobgroup => $jobGroup } ) if ($jobGroup and ref $jobGroup); 

  print STDERR "Got jobs\n";

  my $public_jobs = {} ;
  # my $list = $jobdbm->Job->get_objects( { public => 1 } );
  my @list = map { $_->job } @$jgs ;
